...
p = [int(input()) for _ in range(n)]    #   power of monsters
b = [int(input()) for _ in range(n)]    #   exp from monsters

defeated_count = 0
defeated_monster = [False for _ in range(n)]

for k in range(n):
    for i in range(n):
        if e >= p[i] and defeated_monster[i] == False:
            defeated_monster[i] = True
            e = e+b[i]
